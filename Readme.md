...
If you configure UART Transmitt DMA in Normal mode:  **Must enable USART global interrupt**. **Error:** without interrupts, HAL_UART_Transmit_DMA only transmits for the first time and no more transmittion after. 
...
**Example:** [TestDMA](/TestDMA) is my project using DMA UART Transmit and Receive. This project uses **UART4**, Transmit using **Normal mode** and **Receive using Circular mode**. In the future i will study more about IDLE Mode for more complex data transmit.
  
### UART4 Receive IDLE DMA:

**Expectation:** STM32 will automatically detect if the Receive line is free or not after a serie of incoming data. Then it will jump into an interrupt handler, which gives you some informations: data received, data length.
...
```
**Example code**: in the project [**Final24C04interface**](./Final24C04interface).
...
Therefore you have to fix it by DeInit and then Init it again to make it work properly. Remember to Initialize the module to continue receive:
 ```
